...
mod tests {
    ...
    fn test_program_builder_methods() {
        ...
        let program = Program::new().with_account_struct(Account::new("Account1", "pub"));
        assert_eq!(program.account_structs.len(), 1);
        assert_eq!(program.account_structs[0].name, "Account1");

        let program = Program::new().with_raw_account(RawAccount::new("Raw1", "pub"));
        assert_eq!(program.raw_accounts.len(), 1);
    ...
    fn test_program_module_builder_methods() {
        ...
        let module = ProgramModule::new("MyModule", "pub").with_instructions(instructions);
