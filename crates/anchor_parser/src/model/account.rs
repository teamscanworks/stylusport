...
impl AccountField {
    ...
    pub fn find_constraint(&self, constraint_type: &str) -> Option<&Constraint> {
        self.constraints.iter().find(|c| c.constraint_type == constraint_type)
    }
...
impl RawAccountField {
    ...
    pub fn new(name: impl Into<String>, ty: impl Into<String>, visibility: impl Into<String>) -> Self {
        Self {
            name: name.into(),
            ty: ty.into(),
            visibility: visibility.into(),
...
mod tests {
    ...
    fn test_account_field_builder_methods() {
        ...
        let field = AccountField::new("owner", "Pubkey")
            .with_constraints(vec![constraint1, constraint2]);
